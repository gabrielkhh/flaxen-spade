...
def run():
    ...
    for entry in reversed(entries):
        ...
        if station not in results:
            results[station] = {"weekend": [], "weekday": []}

        if (
            len(results[station]["weekend"]) < 5
            and entry["DAY_TYPE"] == "WEEKENDS/HOLIDAY"
        ):
            if hour not in ("22", "23", "0"):
                results[station]["weekend"].append(
                    {"hour": hour, "tap_in": tap_in, "tap_out": tap_out}
                )

        if len(results[station]["weekday"]) < 5 and entry["DAY_TYPE"] == "WEEKDAY":
            if hour not in ("22", "23", "0"):
                results[station]["weekday"].append(
                    {"hour": hour, "tap_in": tap_in, "tap_out": tap_out}
                )
