...
table_headers = ["Shopping Mall Name", "Nearest MRT Station", "Type of Day", "Hour of Day", "Volume of People"]
table_list_weekday = []
table_list_weekends = []
...
def compute_volume(mall_name, station_name):
    ...
    for hour in hours_tuple:
        ...
        table_list_weekday.append([mall_name, station_name, "Weekday", hour, tap_out_dict[id_weekday]])
        table_list_weekends.append([mall_name, station_name, "Weekends/Holiday", hour, tap_out_dict[id_weekends]])
...
def mall_traffic():
    ...
    for entry in entries:
        try:
            station = TrainStationFactory.load_station(entry['PT_CODE'].split('/')[0])  # an instance of TrainStation
    ...
    for mall_name, coordinates in malls_coordinates.items():
        ...
        list_of_stations = train.raw_location(coordinates["latitude"], coordinates["longitude"]).train_station(limit=3)
        distance, closest_train = list_of_stations[0]  # Tuple of (distance_in_km, Instance of TrainStation)

        # Get the volume of people
        compute_volume(mall_name, closest_train.name)

        mall_data = {"stationName": closest_train.name, "weekday": volume_list_weekday,
