...
def run():
    ...
    mysort = list(
        islice(sorted(tapin, key=lambda x: int(x["TOTAL_TRIPS"]), reverse=True), 50)
    )
    ...
    for entry in mysort:
        ...
        if f"{start}:{end}" not in result:
            result[f"{start}:{end}"] = {"weekend": []}
        result[f"{start}:{end}"]["weekend"].append(
            {"start": start, "end": end, "time": time, "totaltrip": totaltrip}
        )

    print(result)

    with open(
        dataset_path("results/pop_mrt_routes_on_weekends_publicholiday.json"), "w+"
    ) as file:
